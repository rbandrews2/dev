...
export default function GlobalFooter() {
  return (
    <footer className="mt-10 border-t border-amber-500/20 bg-black/60 backdrop-blur-md">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-amber-100/80">
          {footerLinks.map((link) => (
            <Link
              key={link.label}
              to={link.href}
              className="px-3 py-1 rounded-full border border-amber-500/30 bg-black/40 hover:border-amber-400/60 transition"
            >
              {link.label}
            </Link>
          ))}
