...
export function EchoChatWidget({ className }: EchoChatWidgetProps) {
  ...
  if (!user) {
    return (
      <Card className={`bg-black/40 border border-orange-500/30 text-white p-6 ${className}`}>
        <h3 className="text-xl font-semibold text-orange-300 mb-2">EchoChat Messaging</h3>
        <p className="text-orange-100/80">Please sign in to access messaging.</p>
      </Card>
    );
  }

  return (
    <Card className={`bg-black/40 border border-orange-500/30 text-white overflow-hidden ${className}`}>
      <div className="flex min-h-[calc(100dvh-190px)] flex-col md:h-[620px] md:min-h-0 md:flex-row">
        {/* Conversations Sidebar */}
        <div className="flex max-h-[34dvh] min-h-[210px] flex-col border-b border-orange-500/20 md:max-h-none md:min-h-0 md:w-72 md:border-b-0 md:border-r">
          <div className="border-b border-orange-500/20 p-3 sm:p-4">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="flex items-center gap-2 text-base font-semibold text-orange-300 sm:text-lg">
            ...
            <div className="relative">
              ...
              <Input
                ...
                className="h-10 bg-black/30 pl-9 text-sm text-white border-orange-500/20 placeholder:text-orange-400/50"
              />
            </div>
          </div>
          ...
          <ScrollArea className="flex-1">
            {loading ? (
              ...
              <div className="p-4 text-center text-orange-100/60">
              ...
              <div className="divide-y divide-orange-500/10">
                {filteredConversations.map((conv) => (
                  <button
                    ...
                    className={`w-full p-3 text-left transition-colors hover:bg-orange-500/10 ${
                      selectedConversation?.id === conv.id ? 'bg-orange-500/20' : ''
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      {conv.last_message_at && (
                        <span className="shrink-0 text-xs text-orange-100/50">
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {selectedConversation ? (
            <>
                ...
                <div className="space-y-3 sm:space-y-4">
                  {messages.map((msg) => (
                    ...
                    >
                      ...
                      >
                        {msg.sender_email !== user.email && (
                          <p className="text-xs text-orange-300 mb-1">{msg.sender_name || msg.sender_email}</p>
                        )}
              ...
              {/* Message Input */}
              <form onSubmit={handleSendMessage} className="border-t border-orange-500/20 p-3 sm:p-4">
                <div className="flex gap-2">
                  <Input
                    value={newMessage}
                    onChange={(e) => setNewMessage(e.target.value)}
                    placeholder="Type a message..."
                    className="h-11 bg-black/30 text-sm text-white border-orange-500/20 placeholder:text-orange-400/50"
                  />
                  <Button
                    type="submit"
                    disabled={!newMessage.trim() || sending}
                    className="h-11 shrink-0 bg-orange-500/20 px-3 hover:bg-orange-500/30"
            ...
            <div className="flex min-h-[300px] flex-1 items-center justify-center p-6">
              <div className="text-center">
                <MessageSquare className="w-12 h-12 text-orange-400/50 mx-auto mb-3" />
                <p className="text-orange-100/60">Select a conversation to start messaging</p>
              </div>
            </div>
          )}
        </div>
      ...
      {showNewConversation && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <Card className="w-full max-w-sm border-orange-500/30 bg-zinc-900 p-5 sm:p-6">
            ...
            <form onSubmit={handleCreateConversation}>
              <Input
                ...
                className="bg-black/30 border-orange-500/20 text-white mb-4"
              />
