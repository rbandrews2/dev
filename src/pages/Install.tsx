...
export default function Install() {
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <section className="rounded-3xl border border-orange-500/20 bg-[radial-gradient(circle_at_top_right,rgba(249,115,22,0.14),transparent_30%),linear-gradient(180deg,rgba(3,7,18,0.96),rgba(9,9,11,0.98))] p-8">
        <p className="text-xs uppercase tracking-[0.18em] text-orange-200/70">Download and install</p>
        <h1 className="mt-2 text-4xl font-semibold text-white">Install Work Zone OS and activate it with your purchase code.</h1>
        <p className="mt-4 max-w-3xl text-sm text-orange-100/75">
      ...
      <section className="rounded-3xl border border-white/10 bg-black/40 p-6">
        <div className="flex flex-wrap gap-3">
          ...
          <Link to="/activate" className="rounded-xl bg-orange-500 px-4 py-3 font-semibold text-black hover:bg-orange-400">
            Enter activation code
          </Link>
          <Link to="/" className="rounded-xl border border-orange-400/30 px-4 py-3 text-orange-100 hover:bg-white/5">
            Open Work Zone OS
          </Link>
...
function StepCard({ icon, title, body }: { icon: ReactNode; title: string; body: string }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
      ...
      <p className="mt-3 text-sm text-orange-100/75">{body}</p>
    </div>
  );
}
