...
export default function TrainingIndex() {
  ...
  return (
    <div className="min-h-screen text-yellow-100 pb-14">
      <section className="relative overflow-hidden rounded-3xl border border-amber-500/25 shadow-glow p-6 md:p-10 bg-black max-w-6xl mx-auto mt-4">
      ...
      <section className="max-w-6xl mx-auto px-4 mt-8 space-y-6">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          <Link to="/training/videos" className="block">
            <GlassCard className="h-full p-5 flex flex-col gap-3 bg-black/60 border border-amber-500/25 hover:border-amber-400/40 hover:bg-black/65 transition">
              <div className="w-11 h-11 rounded-2xl border border-amber-500/30 bg-black/50 flex items-center justify-center text-amber-300">
              ...
              <div className="mt-auto text-[11px] uppercase tracking-[0.14em] text-amber-200/80 flex items-center gap-2">
                Browse videos
              </div>
            </GlassCard>
          </Link>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {courses.map((c: any) => (
            <Link key={c.course_id} to={`/training/course/${c.course_id}`} className="block">
              <GlassCard className="h-full p-5 flex flex-col gap-4 bg-black/60 border border-amber-500/25 hover:border-amber-400/40 hover:bg-black/65 transition">
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    <p className="text-xs uppercase tracking-[0.18em] text-amber-200/80">{c.category}</p>
                    <h3 className="text-lg font-semibold text-white">{c.title}</h3>
                  </div>

                  <div className="shrink-0 w-11 h-11 rounded-2xl border border-amber-500/30 bg-black/50 flex items-center justify-center text-amber-300">
                    <ShieldCheck className="w-5 h-5" />
                  </div>
                </div>

                <p className="text-sm text-amber-100/75 line-clamp-3">
                  {c.description}
                </p>

                <div className="mt-auto flex items-center justify-between text-xs text-amber-200/80">
