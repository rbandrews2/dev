  ...
  return (
    <div className="space-y-6 text-amber-50">
      ...
      <GlassCard className="p-5 space-y-5">
        {!enabled && (
          <div className="rounded-lg border border-amber-500/30 bg-black/50 px-3 py-2 text-sm text-amber-100/80">
            Video conferencing is locked by config, but you can still preview a meeting below.
          </div>
        )}

        <div className="grid gap-3 md:grid-cols-3">
          <label className="flex flex-col text-sm text-amber-100/90">
            Meeting ID
            <input
              type="text"
              value={meetingId}
              ...
              className="mt-1 rounded-md bg-black/70 border border-amber-500/30 px-3 py-2 text-amber-50 placeholder:text-amber-200/50 focus:outline-none focus:ring-1 focus:ring-amber-400"
            />
          </label>
        ...
        <div className="flex justify-end">
          <button
            ...
            className="px-4 py-2 rounded-md bg-amber-500 text-black font-semibold hover:bg-amber-400 transition"
          >
            Start Video
          </button>
        </div>
        ...
        {joinUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 text-xs text-amber-200 underline hover:text-amber-100"
          >
